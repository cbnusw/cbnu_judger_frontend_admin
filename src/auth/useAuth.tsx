...
interface UseAuth {
  signin: (no: string, password: string) => Promise<void>
  signup: (no: string, password: string) => Promise<void>
  signout: () => void
}
...
export function useAuth(): UseAuth {
  ...
  async function authServerCall(urlEndpoint: string, no: string, password: string): Promise<void> {
    try {
      ...
      if ('user' in data && 'token' in data.user) {
        toast({
          title: `Logged in as ${data.user.no}`,
          status: 'info',
        })
